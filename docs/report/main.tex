...
\textbf{(Extension)} We integrate diversity-guided sampling and ReEvo reflective
evolution on top of deduplication inside the same pipeline and analyse their
stacked behaviour; reflection-only is not separately ablated (\S\ref{sec:lim}~\#4),
...
condition's low mean ($-209.66$) and variance ($0.59$), with the late-breakthrough
pattern (Appendix~\ref{app:reevo-figure}), are qualitatively consistent with
\citet{ye2024reevo}'s landscape-smoothing mechanism; reflection cannot be
isolated here as it runs with dedup and diversity active.
