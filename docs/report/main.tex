...
\]
where $\widehat{\mathrm{Perf}}$ and $\widehat{\mathrm{Div}}$ are the cluster's
score and diversity term, each min-max normalised across the island's clusters.
$\mathrm{Div}(c)$ is the row-centred cosine distance between $c$'s representative
$375$-dim fingerprint and the per-island fingerprint mean. The exploration weight
...
\S\ref{sec:lim}~\#4). Total API usage across all runs is $\sim 2{,}100$ calls.
...
(\S\ref{sec:fp}) shows $98.5\%$ of genuinely different fingerprint pairs at
cosine $>0.98$, so Level~2 is disabled; Hamming/edit-distance or learned
metrics are prerequisites for any high-dimensional behavioural similarity filter.
...
late-breakthrough pattern (\S\ref{sec:results}, Appendix~\ref{app:reevo-figure}), are qualitatively
...
decisions as a different behavioural proxy, sitting between syntactic AST-hash
dedup and undecidable semantic equivalence.

\section{Limitations}
\label{sec:lim}

\textbf{(1) Statistical power:} $n{=}2$--$4$ for the main table and $n{=}1$
for BCR/t-SNE; results are exploratory rather than significance-tested.
\textbf{(2) Probe generalisation:} fingerprint equivalence is not verified
end-to-end on full OR3 (O2 is indirect evidence). \textbf{(3) Single
$\mathrm{Div}$ metric:} only row-centred cosine; edit distance, learned
embeddings, and per-program filter-level rejection are future work.
\textbf{(4) Reflection-only not ablated:} \textit{reevo} stacks reflection on
dedup and diversity-guided sampling; a reflection-only run is needed to isolate
it. \textbf{(5) Sandbox bypass:} probes use \texttt{exec()} with $5$\,s timeout
rather than the forked sandbox.

\section{Conclusion}
\label{sec:conc}

Behavioural-fingerprint deduplication cuts FunSearch's samples-to-best by $38\%$
on OR3 without degrading best score. Exploratory BCR ($n{=}1$) suggests
diversity-guided sampling may slow cluster coverage; full-stack ReEvo gives the
lowest-variance condition ($n{=}4$). Future work: richer $\mathrm{Div}$ metrics,
reflection-only ablation, larger sample budgets.
