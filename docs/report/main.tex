...
\texttt{\{sijiechan2,wbian9\}-c@my.cityu.edu.hk}}
...
equivalent heuristics: on an unmodified run we observe a $45\%$ natural duplicate rate,
wasting costly API calls and sandbox evaluations.
...
offline \textbf{Behavioural Coverage Rate (BCR)} analysis---introduced as a lightweight
tool for characterising exploration behaviour---shows the reverse-intuitive finding that
deduplication \emph{alone} drives broader behavioural coverage than deduplication
combined with filter-level diversity. Adding ReEvo on top yields the lowest-variance
condition in our runs (mean $-209.66$, std $0.59$, $n{=}4$), consistent with the
landscape-smoothing mechanism reported by \citet{ye2024reevo}.%
...
\emph{identical} to programs already in the archive, indicating a high rate of
behaviourally redundant proposals that syntactic hashing does not catch. This matches
...
evolution on top of deduplication inside the same pipeline and characterise their
marginal effects (the complete
$\mathrm{Score}(c)=\mathrm{Perf}(c)+\beta\cdot\mathrm{Div}(c)$ cluster scoring is
future work). \textbf{(Tool)} An offline \textbf{Behavioural Coverage Rate} (BCR)
metric inspired by MAP-Elites \citep{mouret2015illuminating} reveals that
deduplication alone reaches full behavioural coverage faster than deduplication
combined with filter-level diversity. Together these findings suggest that, at the
behaviour level, \emph{removing redundancy} and \emph{promoting novelty} are not
simply additive \citep{vanstein2025llm}.
...
All experiments use OR-Library OR3 for evaluation and Weibull-5k as auxiliary data,
following the FunSearch evaluation protocol \citep{romera2023mathematical}. The LLM
...
diversity, $n{=}4$), and \textit{reevo} (funnel plus ReEvo reflection, $n{=}4$). Total
API usage across all runs is $\sim 2{,}100$ calls.
...
scores in $-210\pm 1$. We therefore conclude that, on this budget, neither
deduplication, filter-level diversity, nor reflection imposes a measurable
best-score cost --- a finding that is consistent with (but, per \S\ref{sec:disc}
Insight~2, does not directly verify) a low false-positive rate on the full input
distribution.

\textbf{(O3) ReEvo is the lowest-variance condition.} ReEvo is the only condition
whose mean is below $-210$ ($-209.66 \pm 0.59$). This is consistent with the
landscape-smoothing mechanism attributed to reflection by \citet{ye2024reevo}. We
avoid strong causal claims, since $n{=}4$ is insufficient for significance testing.

\textbf{(O4) Dup rates decrease with extra signals.} Among dedup-equipped conditions,
the rejection rate drops from $0.401$ (dedup) to $0.342$ (dedup+div) to $0.314$
(reevo), indicating that filter-level diversity and reflection push the LLM toward
genuinely new behaviours relative to the archive. (We do not include baseline in this
trend because it does not route candidates through the dedup filter; the $45\%$
natural duplicate rate mentioned in \S\ref{sec:intro} is computed independently on
identical-score counts.)
...
\caption{Behavioural Coverage Rate over sample index. Dedup-only (blue) reaches
full coverage (BCR$=1$) at sample $51$; Dedup+Div (orange) reaches full coverage at
sample $65$. Only these two conditions export fingerprints in the current run set.}
\label{fig:bcr}
\end{figure}
...
sample $65$. \textbf{Reverse-intuitive finding:} filter-level diversity \emph{slows}
cluster visitation. Once duplicates are filtered, retained samples are already
diverse by construction; the extra diversity constraint then over-constrains the
proposer, concentrating exploration within already-reached clusters. This exposes a
mechanism-level difference not visible in the best-score curve
...
\textbf{Deduplication subsumes first-order diversity.} The $38\%$ samples-to-best
reduction and the BCR curves together show that removing behavioural redundancy
already delivers the exploration effect that explicit diversity mechanisms target;
stacking filter-level diversity on top slows cluster visitation and spreads points
within clusters rather than reaching new ones. This echoes Novelty
Search's observation that penalising redundancy can substitute for rewarding
novelty directly \citep{lehman2011novelty}.
...
\textbf{Reflection consistent with landscape smoothing.} ReEvo's low mean
($-209.66$) and low variance ($0.59$), together with the late-breakthrough pattern
(\S\ref{sec:breakthrough-exp}), are qualitatively consistent with
\citet{ye2024reevo}'s landscape-smoothing mechanism; $n{=}4$ precludes a causal claim.
...
\textbf{(5) BCR/t-SNE scope:} Dedup and Dedup+Div only.

\section{Conclusion}
\label{sec:conc}

Behavioural-fingerprint deduplication cuts FunSearch's samples-to-best by $38\%$ on
OR3 online bin packing without degrading best score; BCR analysis reveals that
filter-level diversity \emph{slows} cluster coverage, while ReEvo gives the
lowest-variance condition, consistent with landscape smoothing. Future work:
adaptive-$\beta$ cluster scoring and larger sample budgets.
