...
\begin{document}
\maketitle
\lhead{}  % suppress the template's "Published as ICLR 2024" running header for this course submission
...
identically to an ancestor---a conservative proxy for behavioural redundancy whose
sandbox evaluations carry no new information (and, under bounded budgets, also
waste future API quota).
...
evaluation; we further integrate diversity-guided sampling and ReEvo reflective evolution
...
than deduplication combined with diversity-guided sampling---a hypothesis flagged for
multi-seed follow-up. Adding ReEvo on top of deduplication and diversity-guided sampling
...
We report three results, all under a $150$-sample budget. \textbf{(Main)} A three-level
...
\textbf{(Extension)} We integrate diversity-guided cluster sampling
($\mathrm{Score}(c)=\mathrm{Perf}(c)+\beta\cdot\mathrm{Div}(c)$, \S\ref{sec:div})
and ReEvo reflective evolution on top of deduplication inside the same pipeline
and analyse their stacked behaviour; reflection-only is not separately ablated
(\S\ref{sec:lim}~\#4). \textbf{(Tool)} We introduce an offline \textbf{Behavioural Coverage Rate} (BCR)
...
diversity-guided sampling, consistent with the view that at the behaviour level
...
\emph{before} the expensive sandbox, and optionally wires in diversity-guided sampling
...
diversity-guided sampling; archived programs feed the LLM via prompt
...
inputs in \S\ref{sec:disc} and \S\ref{sec:lim}~\#2).
...
result (\S\ref{sec:disc}, ``Cosine on discrete decision vectors is degenerate'').
...
\subsection{Diversity-Guided Cluster Sampling}
\label{sec:div}

When \texttt{DIVERSITY\_ENABLED=1}, the prompt-construction step inside each
island's \texttt{get\_prompt()} (in \texttt{programs\_database.py}) selects $k{=}2$
clusters via softmax over a \emph{combined} score
\[
\mathrm{Score}(c) \;=\; \widehat{\mathrm{Perf}}(c) \;+\; \beta\cdot\widehat{\mathrm{Div}}(c),
\]
with both terms min-max normalised across the island's clusters and
$\mathrm{Div}(c) = 1 - \overline{\cos}(\tilde{\mathbf{s}}_c, \tilde{\mathbf{s}}_{c'})_{c'\ne c}$
the row-centred mean cosine distance from $c$ to the other clusters; here
$\mathbf{s}_c \in \mathbb{R}^{|\mathcal{D}|}$ ($|\mathcal{D}|{=}20$) is the per-OR3-file
score signature already used as the archive's bucket key (so $\mathrm{Div}$ is
defined on score patterns, not on the $375$-dim probing fingerprint). Row-centring
decouples it from absolute level, which $\widehat{\mathrm{Perf}}$ already captures.
The exploration weight
$\beta$ decays linearly from $\beta_0{=}0.3$ to $0$ over the first $350$ accepted
programs, so early sampling prefers behaviourally under-represented clusters and
late sampling reverts to pure performance-driven exploitation. This is the
mechanism activated in our \emph{dedup+div} and \emph{reevo} runs.
...
All experiments evaluate on the OR3 dataset only; Weibull-5k from the original
FunSearch protocol \citep{romera2023mathematical} is not used in this report. The LLM
...
(fingerprint funnel only, $n{=}4$), \textit{dedup+div} (funnel plus
diversity-guided sampling, $n{=}4$), and \textit{reevo} (funnel + diversity-guided sampling + ReEvo
reflection stacked, $n{=}4$; reflection-only is not separately ablated---see
\S\ref{sec:lim}~\#4). Across all runs the proposer issues $\sim 2{,}100$ samples;
the four \emph{reevo} runs add up to one reflection call per accepted program.
...
ReEvo condition is the only one whose mean is above $-210$.}
\label{fig:conv}
\end{figure}
...
measurable best-score cost from deduplication, diversity-guided sampling, or reflection
--- an observation consistent with (but, per \S\ref{sec:disc}, ``Cosine on discrete decision vectors is degenerate'', not a direct
...
(dedup $+$ diversity $+$ reflection) is the only one whose mean is above $-210$
...
stacked on top of dedup and diversity-guided sampling in these runs, we cannot
...
sample $65$. \textbf{Counterintuitively,} under this single pair of runs,
diversity-guided sampling appears to \emph{slow} cluster visitation. A plausible
...
All four ReEvo runs plateau early and break through at sample $87$--$123$,
consistent with the reflection buffer needing time to accumulate high-scoring
strategies (per-run curves in Appendix~\ref{app:reevo-figure}; $n{=}4$ precludes
a causal claim).
...
and that stacking diversity-guided sampling on top can in fact slow cluster
...
\textbf{Cosine on discrete decision vectors is degenerate.} Calibration
(\S\ref{sec:fp}) shows $98.5\%$ of distinct-pair similarities $>0.98$, so
Level~2 is disabled; Hamming/edit-distance or learned metrics are needed.

\textbf{Reflection-plus-stack consistent with landscape smoothing.} The full-stack
condition's highest mean ($-209.66$) and lowest variance ($0.59$), with the
late-breakthrough pattern (Appendix~\ref{app:reevo-figure}), are qualitatively
consistent with \citet{ye2024reevo}'s landscape-smoothing mechanism; reflection
cannot be isolated here as it runs with dedup and diversity active.
...
introduces LLM reflection between generations; we integrate it inside the
FunSearch loop. BCR is inspired by MAP-Elites
\citep{mouret2015illuminating}/Novelty Search \citep{lehman2011novelty} but
measures cluster visitation over time; van Stein et al.~\citep{vanstein2025llm}
advocate runtime-trajectory behaviour-space analysis. Our probe-decision
fingerprint sits between syntactic AST-hash dedup and undecidable semantic
equivalence.

\section{Limitations}
\label{sec:lim}

\textbf{(1) Statistical power:} $n{=}2$--$4$ (main table), $n{=}1$ (BCR/t-SNE);
exploratory, not significance-tested. \textbf{(2) Probe generalisation:}
fingerprint equivalence is not verified end-to-end on full OR3. \textbf{(3)
Single $\mathrm{Div}$ metric:} row-centred cosine only; edit distance, learned
embeddings, and per-program filter-level rejection are future work. \textbf{(4)
Reflection not isolated and probes use \texttt{exec()} (5\,s timeout) rather
than the forked sandbox.}

\section{Conclusion}
\label{sec:conc}

Behavioural-fingerprint dedup cuts samples-to-best by $38\%$ on OR3 with no
best-score loss; exploratory BCR ($n{=}1$) suggests diversity-guided sampling
slows cluster coverage; full-stack ReEvo gives the lowest variance ($n{=}4$).
...
the course-provided \texttt{gpt-5-nano} endpoint. Per-condition raw run logs
(\texttt{funsearch-baseline/logs/layer*/run\_log.csv}) and the aggregation
script \texttt{scripts/collect\_results.py} are included in the released
repository; the reported means and standard deviations are reproduced exactly
by that script. (The LLM API is non-deterministic, so we do not claim
seed-level reproducibility of fresh runs; the released CSV logs are the
authoritative artefact.)

\textbf{API robustness.} The proposer LLM wrapper uses a $60$\,s socket
timeout and at most $5$ exponential-backoff retries, configurable via
\texttt{API\_TIMEOUT} and \texttt{API\_MAX\_RETRIES}. The ReEvo reflection
helper is a separate code path with a fixed $30$\,s timeout and no retry
loop; reflection failures are non-fatal (the run continues without storing
that reflection). The proposer bound was added on 2026-04-21 to cap
wall-clock on re-runs; it does not affect the logged $150$-sample results,
which completed under a previous, unbounded retry configuration. The Colab notebook linked in the abstract regenerates every
...
\section{Per-Run ReEvo Convergence}
\label{app:reevo-figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\linewidth]{reevo_breakthrough.pdf}
\caption{Per-run ReEvo convergence curves ($n{=}4$). All four runs show a late
improvement at roughly sample $87$--$123$, after an initial plateau.}
\label{fig:reevo}
\end{figure}
